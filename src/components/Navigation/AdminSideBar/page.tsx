...
const AdminSideBar: React.FC = () => {
  ...
  const useCourseStoreObj = {
    courseName: useStore(useCourseStore, (state) => state.name),
    coursesList: useStore(useCourseStore, (state) => state.coursesList),
  ...
  useEffect(() => {
    if (!!useCourseStoreObj.coursesList && !!useCourseStoreObj.courseName) {
      const course = useCourseStoreObj.coursesList.filter((course) =>
        !!useCourseStoreObj.courseName
          ? course.name?.toLocaleLowerCase() ===
            useCourseStoreObj.courseName.toLocaleLowerCase()
          : course
      )[0];
  ...
  }, [useCourseStoreObj.courseName, useCourseStoreObj.coursesList]);

  useEffect(() => {
    const fetchData = async () => {
      if (
        useUserStoreObj.userRole === TypesOfUser.ADMIN &&
        useCourseStoreObj.coursesList &&
        !!!useCourseStoreObj.coursesList[0]._id
      ) {
        try {
  ...
  }, [useCourseStoreObj.coursesList, useUserStoreObj.userRole]);
  ...
  return (
    <>
      ...
      useUserStoreObj.isLoggedIn ? (
        <div className='flex h-screen min-w-[15%] 2xl:min-w-[10%] flex-col justify-center border-r-2 border-duoGray-light bg-duoGray-lighter font-extrabold tracking-wide text-duoGray-darker dark:border-duoGrayDark-light dark:bg-duoBlueDark-darkest dark:text-duoGrayDark-lightest 3xl:w-[10%]'>
          <label
          ...
          <div className='flex items-center justify-center border-b-2 dark:border-duoGrayDark-light'>
            <ul className='w-full cursor-default text-center text-[0.75rem] uppercase md:text-[0.9rem] lg:text-lg '>
              {useCourseStoreObj.coursesList ? (
                useCourseStoreObj.coursesList.length > 0 &&
                !!useCourseStoreObj.coursesList[0]._id ? (
                  // coursesList.map((item,index)=><></>)
                  useCourseStoreObj.coursesList.map((item, index) => (
                    <li
                      key={index}
                      className={`p-3
                       ${
