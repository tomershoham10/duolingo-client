...
const AdminLevel: React.FC<AdminLevelProps> = ({
  ...
  const handleAddExercise = (levelId: string) => {
    updateSyllabusFieldId(levelId);
  ...
  return (
    <div className="flex flex-col gap-4 px-4">
      {availableLevelData.map((level: LevelType, idx: number) => {
        ...
        const exercisesCount = level.exercisesIds?.length || 0;
        const exercisesList = courseDataState.exercises.find((e) => e.fatherId === level._id)?.data || [];
                ...
                {exercisesList.length > 0 ? (
                  <div className="ml-4">
                    ...
                    <button 
                      ...
                      onClick={() => handleAddExercise(level._id)}
                    >
